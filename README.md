...
- Runs Gemini in **one single bulk request** over all rows in `candidates.csv`, then writes enriched output to `data/gemini_processed.csv`.
...
- `AI_REQUEST_TIMEOUT_SEC` (default `60`; increase to `90` if your connection is slow)
...
- Gemini analysis is executed as a single bulk request for all candidates in the run (to minimize per-row request overhead).
- AI network timeouts/connection issues are retried automatically with backoff; final failures are recorded per-row in `ai_error` instead of crashing the full run.
